	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=1;i<=excel.rowCount("MasterTestCases");i++){		
			...
			if(excel.getData("MasterTestCases", i, 2).equalsIgnoreCase("Y")){
				...
				report=new ExtentReports("D:\\Batch82\\StockAccounting_Hybrid\\Reports\\"+TCModule+FuntionLibrary.curdate()+".html");
				...
				for(int j=1;j<=excel.rowCount(TCModule);j++){	
					...
					try{
						if(Function_Name.equalsIgnoreCase("startBrowser")){
							...
							FuntionLibrary.captureData(driver, Locator_Type, Locator_Value);
							test.log(LogStatus.INFO, Description);
						}
						else if(Function_Name.equalsIgnoreCase("tableValidation")){
							FuntionLibrary.tablevalidation(driver, Test_Data);
							test.log(LogStatus.INFO, Description);
						}
						...
						FileUtils.copyFile(srcFile, new File("D:\\pintu\\StockAccountingHybrid\\ScreenShot\\Screenshots\\"+Description+FuntionLibrary.curdate()+".png"));
